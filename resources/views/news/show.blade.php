...
<div class="container">
    ...
    <div class="left-content">
        @if ($news->image)
            <img src="{{ asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="news-image-detail article-image-detail">
        @endif

        <h1 class="news-title article-title">{{ strtoupper($news->title ?? 'Judul tidak ditemukan') }}</h1>

<div class="news-meta article-meta">
    <span>Diterbitkan: {{ $news->updated_at ? $news->updated_at->format('D, M d Y') : 'Tanggal tidak tersedia' }}</span>
</div>
    ...
    {!! $news->content ?? '<p>Konten tidak tersedia</p>' !!}
        ...
        <div class="recommended">
            ...
            <div class="recommended-list">
                @foreach($randomNews as $rec)
                    <a href="{{ route('news.show', $rec->id) }}" class="recommended-horizontal-card">
                        @if($rec->image)
                            <img src="{{ asset('storage/' . $rec->image) }}" alt="{{ $rec->title }}">
                        @endif
                        <div class="recommended-horizontal-content">
                            <div class="title">{{ Str::limit($rec->title, 60) }}</div>
        ...
        <div style="margin-top: 30px;">
            <a href="{{ route('news.index') }}" class="btn-back">← Lihat Daftar Berita</a>
        </div>
    </div>
        ...
        <div style="margin-top: 30px;">
            ...
            <ul class="sidebar-list">
                @foreach($latestNews as $latest)
                    <li><a href="{{ route('news.show', $latest->id) }}">{{ $latest->title }}</a></li>
                @endforeach
            </ul>
        </div>
    </div>
